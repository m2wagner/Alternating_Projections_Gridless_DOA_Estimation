...
str = strcat('Generalized Null Spectrum');
...
% fun2        = @(theta) abs( (exp(1i*pi*sind(theta).*r)'*G*exp(1i*pi*sind(theta).*r)) );
fun2        = @(theta) abs(null_spec_polynomial(G,r, exp(1i*pi*sind(theta))));
max_theta   = 90;
thetas      = -max_theta:.1:max_theta;
Null_spec   = zeros(length(thetas),1);
for i = 1:length(thetas)
    Null_spec(i)   = fun2(thetas(i));
end


figure(1)
subplot(4,2,7)
plot(thetas,Null_spec,'LineWidth',2)
hold on
% plot(90*[1 1],[min(Null_spec) max(Null_spec)],'r')
% plot(-90*[1 1],[min(Null_spec) max(Null_spec)],'r')
% plot(-180*[1 1],[min(Null_spec) max(Null_spec)],'r')
scatter(DoA_deg,zeros(K,1),125,'r','x','LineWidth',2)
plot(+180*[1 1],[min(Null_spec) max(Null_spec)],'r')
axis([-max_theta,max_theta,0,M])
grid on
title('Evaluated on unit circle','FontSize',18)
xlabel('$-\sin^{-1}(\frac{\angle z}{\pi})^{\circ}$','interpreter','latex','FontSize',14)
ylabel('Magnitude','FontSize',14)
text(-110,M-1, 'c)', 'FontSize', 18)
...
hold on
scatter(real(exp(1i*pi*sin(DoA_rad))),imag(exp(1i*pi*sin(DoA_rad))),125,'r','x','LineWidth',2)
% legend('Null Spectrum','Complex unit circle','True DoA','Location','southwest')
axis equal
colormap parula
xlabel('Real','FontSize',14)
% ylabel('Imaginary','FontSize',14)
